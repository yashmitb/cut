...
// Combined initial-load endpoint: profile + day's food in ONE request
// (one auth check, one DB connection) instead of separate round trips.
export async function GET(req: NextRequest) {
  try {
    ...
    const [profileRows, items] = await Promise.all([
